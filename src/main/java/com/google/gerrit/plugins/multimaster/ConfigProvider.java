...
public class ConfigProvider implements Provider<Config> {
  ...
  @Inject
  ConfigProvider(SitePaths site, @PluginName String pluginName)
      throws ConfigInvalidException, IOException {
    config = load(site, pluginName);
  }

  private static Config load(SitePaths site, String pluginName)
      throws ConfigInvalidException, IOException {
    String configPath = site.etc_dir + "/plugins/" + pluginName + ".config";
    log.info("Loading configuration from " + configPath);
    FileBasedConfig cfg =
        new FileBasedConfig(new File(configPath), FS.DETECTED);
